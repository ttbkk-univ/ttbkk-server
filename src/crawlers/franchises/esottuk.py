...
class EsottukCrawler(BaseCrawler):
    ...
    def get_place_data(self):
        try:
            elements = self.driver.find_elements_by_xpath('//*[@id="addList"]/ul/li')
        except:
            print('추가 데이터가 없습니다.')
            return []
        places = []
        for element in elements:
            place_name = str(element.find_element_by_xpath('./div[1]/a/div/p[1]').text).replace('이소 ', '')

            name = '%s %s' % (self.brand_name, place_name)
            print(name)
            address = element.find_element_by_xpath('./div[1]/a/div/p[2]').text
            latitude, longitude = get_latlng(address, name)
            if not latitude or not longitude:
                print('[failed] %s\n%s' % (name, address))
                continue
            telephone = str(element.find_element_by_xpath('./div[1]/a/div/p[3]').text)
