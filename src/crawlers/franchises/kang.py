...
class KangCrawler(BaseCrawler):
    ...
    def get_place_data(self):
        try:
            elements = self.driver.find_elements(by=By.XPATH, value=('//*[@id="wrap"]/div[15]/div[2]/ul/li'))
        except:
            return []
        places = []
        for element in elements:
            place_name = str(element.find_element(by=By.XPATH, value='./a/div[2]/p[1]').text)
            name = '%s %s' % (self.brand_name, place_name)
            try:
                address = element.find_element(by=By.XPATH, value='./a/div[2]/p[2]').text.split('주소 : ')[1]
            except:
                address = None
            try:
                telephone = element.find_element(by=By.XPATH, value='./a/div[2]/p[3]').text.split('TEL : ')[1]
            except:
                telephone = None
            print(name)
